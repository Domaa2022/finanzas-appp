...
export function SavingsOverview({ goals }: SavingsOverviewProps) {
  ...
  if (activeGoals.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400 dark:text-slate-500 text-sm">
        ...
        <Link href="/ahorros" className="text-emerald-600 dark:text-emerald-400 hover:underline mt-1 block text-xs">
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        return (
          <div key={goal.id}>
            <div className="flex justify-between text-sm mb-1">
              <span className="font-medium text-gray-700 dark:text-slate-300 truncate">{goal.nombre}</span>
              <span className="text-gray-400 dark:text-slate-500 shrink-0 ml-2">{pct}%</span>
            </div>
      ...
      {goals.length > 4 && (
        <Link href="/ahorros" className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline text-center">
